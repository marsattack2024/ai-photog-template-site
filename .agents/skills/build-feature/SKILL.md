...
Step 6:   Verification (.Codex/rules/verification.md — 17 checks)
...
2. Read `AGENTS.md` — refresh on workflow rules and verification checklist
...
- After all parallel tasks complete, run the semantic conflict detector before the TypeScript check:
  `bash .Codex/hooks/parallel-conflict-check.sh <commit-before-parallel-started>`
...
Run the 20-point verification checklist from `.Codex/rules/verification.md` before claiming the feature is done.
...
- **Logging compliance in new API routes** — subagents don't auto-load `.Codex/rules/api-routes.md`, so check every new `route.ts` for `createRouteLogger` and the 8 canonical events. This is the most common subagent compliance gap.
...
- If all PASS: note it and proceed to security gate. Create the marker so the stop hook knows: `touch .Codex/ralph/ui-checked`
- The full checklist is in the `/ui-check` skill.
...
- Email: `bash .Codex/scripts/notify.sh "Plan approved — starting execution" "Plan approved for [feature]. Starting execution of N tasks."`
...
- Email: `bash .Codex/scripts/notify.sh "Build complete — ready for merge" "Feature [name] is complete and verified. Ready for your merge/PR decision."`
...
- Email: `bash .Codex/scripts/notify.sh "Blocked — needs your input" "Blocked on [task] after 3 attempts. Need your direction."`
