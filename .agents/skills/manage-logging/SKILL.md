...
3. Add the 8 required logging events (per `docs/logging-standards.md`):
   ...
   | `llm:start` | Before Codex call | info | `model`, `max_tokens` |
   | `llm:done` | After Codex response | info | `inputTokens`, `outputTokens`, `elapsedMs`, `tokensPerSec` |
...
2. For each LLM route, verify all 8 canonical events are present:
   ...
   - [ ] `"llm:start"` — before Codex call (model, maxTokens)
...
|-------|---------|---------|
| `error` | Unrecoverable failures | DB write failed, Codex API 500 |
