...
3. For each changed item, run the update command:
   ```bash
   npx tsx .claude/tools/notion-gsd.ts update "Feature Name" --status "Done" --progress "100%"
   npx tsx .claude/tools/notion-gsd.ts update "Feature Name" --status "In Progress" --progress "50%"
   npx tsx .claude/tools/notion-gsd.ts update "Feature Name" --status "Pending" --progress "0%"
   npx tsx .claude/tools/notion-gsd.ts update "Feature Name" --status "Blocked" --blocking "Reason here"
   ```
4. If a completed item isn't in the board yet, add it first:
   ```bash
   npx tsx .claude/tools/notion-gsd.ts add "Feature Name" --status "Done" --category "GTM" --priority "High" --progress "100%"
   ```
...
Is it a permanent project convention or rule?
  ├─ Yes, applies to all files → CLAUDE.md
  ├─ Yes, scoped to specific file types → .claude/rules/ with paths: frontmatter
  └─ No ↓

Is it a pattern, insight, or debugging tip Claude discovered?
...
3. **Check `.claude/rules/`** — should any session learning become a path-scoped rule?
4. **Check `CLAUDE.md`** — does anything need updating (test count, file paths, inventory)?

### Apply Changes

- Update the appropriate file(s) directly
- For `.claude/rules/`: include `paths:` frontmatter if the rule only applies to specific file types
...
2. **Apply the quality gate.** A rule earns a place in lessons.md only if ALL of these are true:
   ...
   - It would prevent a real bug if Claude read it before writing code
...
**Skill Gaps** — Things Claude got wrong, needed multiple attempts, or struggled with:
...
- *Action: Write a rule in lessons.md or .claude/rules/ that prevents recurrence*
...
- *Action: Add to CLAUDE.md workflow rules or create a new .claude/rules/ file*

**Missing Knowledge** — Facts about the project that Claude didn't know but should have:
...
- *Action: Add to MEMORY.md or the appropriate .claude/rules/ file*
