---
name: ralph
description: Activate, check, or stop the Ralph autonomous iteration loop. Ralph keeps Claude working on a task across multiple turns until a completion promise is found, without the user needing to re-prompt. Use when starting a long task that should run autonomously (e.g. "implement feature X end-to-end"). Commands: start <task>, status, stop.
user-invocable: true
...
Ralph is the autonomous iteration loop. When active, it intercepts Claude's Stop event and re-feeds the task prompt, keeping Claude working until it outputs a completion promise tag. Think of it as "set it and forget it" for long multi-step tasks.

**The loop:** Claude works → tries to stop → Ralph checks for promise → not found → re-injects prompt → Claude keeps working → eventually outputs `<promise>PROMISE_TAG</promise>` → Ralph detects it → loop ends cleanly.

**State file:** `.claude/ralph/state.json` — exists = loop active, deleted = loop inactive.
...
3. Create `.claude/ralph/state.json` with:
   ```json
   {
   ...
   ```
4. Create `.claude/ralph/NOW.md` with the initial task state:
   ```markdown
   # Current State
...
6. Report to the user:
   ...
   - That Claude will now begin working autonomously
...
3. If yes:
   ...
   - Delete `.claude/ralph/NOW.md` (if it exists)
...
Ralph's stop hook scans for this tag. Finding it → Ralph deletes the state file and lets Claude exit normally.
...
`.claude/ralph/NOW.md` is a live scratchpad that survives context compression. Update it at these moments:
...
Claude begins working immediately...
...
Ralph stop hook detects promise → deletes state.json + NOW.md → Claude exits normally
```
